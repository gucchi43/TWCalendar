...
class CalendarViewController : UIViewController {
    ...
    func generateCalendar() {
        ...
        var TWValues = [String]()
        print("viewID : ", viewID)
        let TWData = TWDayArrayData.sharedSingleton.TWDayDic
        if let values = TWData[viewID] {
            TWValues = values
            print("TWValues", TWValues)
        }
        ...
        for i in 0...41{
            ...
            if (i < dayOfWeek - 1) {
                ...
                let twitterTag = String(format: "%04d", year) + String(format: "%02d", month) + String(format: "%02d", tagNumber) + "&t"
                print("i ,twitterTag :", i , twitterTag)
                if TWValues.indexOf(twitterTag) != nil {
                    print("この日はTweetしてるぜ:", twitterTag)
